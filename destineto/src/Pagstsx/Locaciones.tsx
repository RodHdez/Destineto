...
const cabanaLeft = [
  <div className="lo-page-content lo-page-content--centered">
    ...
    <div className="lo-photo-placeholder" aria-label="Foto de La Cabaña del Tío Neto">
      <span className="lo-photo-label">📷<br />Foto de<br />La Cabaña</span>
    </div>
  </div>,
...
const cabanaRight = [
  <div className="lo-page-content">
    <p className="lo-drop-cap">
      Establecida entre los frondosos bosques y las calmadas nieblas de Los Naranjos, La Cabaña del Tío Neto es un refugio construido para simular la manera de antes; paredes blancas recubiertas de cal pura, sin adornos innecesarios, honestas y sólidas como la mano que las levantó.
    </p>
    <p className="lo-text">
...
const barrinaLeft = [
  <div className="lo-page-content lo-page-content--centered">
    ...
    <div className="lo-vintage-divider" aria-hidden="true">✦ ✦ ✦</div>
    <div className="lo-photo-placeholder" aria-label="Foto de La Barriña">
      <span className="lo-photo-label">📷<br />Foto de<br />La Barriña</span>
    </div>
  </div>,
...
const barrinaRight = [
  <div className="lo-page-content">
    <p className="lo-drop-cap">
      Establecida en la costa de La Barra de Santiago, La Barriña es un refugio tropical donde el sol veraniego y el sonido calmado de las olas marcan el ritmo del día. Ya sea que te refresques en la piscina comunitaria o te adentres en las cálidas aguas de la playa, cada momento aquí se siente como una celebración tranquila de la vida.
    </p>
    <p className="lo-text">
...
function Locaciones() {
  ...
  const flipAnimation = (forward: boolean, then: () => void) => {
    ...
    setTimeout(() => {
      ...
      left.style.transform = forward ? 'perspective(1200px) rotateY(0deg)' : 'perspective(1200px) rotateY(-90deg)'
      setTimeout(() => {
  ...
  const goNext = () => {
    if (page >= currentRight.length - 1) return
    flipAnimation(true, () => setPage(p => p + 1))
  }

  const goPrev = () => {
    if (page <= 0) return
    flipAnimation(false, () => setPage(p => p - 1))
  }

  const switchLocation = (idx: number) => {
    if (idx === locationIdx) return
    flipAnimation(idx > locationIdx, () => {
      setLocationIdx(idx)
      setPage(0)
    })
  }
  ...
  return (
    <div className="lo-scene">
      ...
      <div className="lo-journal open-book flip-journal">
        ...
        {/* Side bookmarks — site navigation */}
        ...
        {/* Bottom tabs — switch between locations */}
        <nav className="lo-location-tabs" aria-label="Cambiar locación">
          {locations.map((loc, i) => (
            <button
              key={loc.id}
              className={`lo-location-tab ${i === locationIdx ? 'lo-location-tab--active' : ''}`}
